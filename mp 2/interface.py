...
def letter_appending(guess,word):
	letter_in_word=pyglet.text.Label(guess, font_size=25, x=width//2-(((len(word)-30)*44)//2+(8//2)*(len(word)-31))+(1-30)*(44+8)+44//2, y=15+50//2, anchor_x='center', anchor_y='center', color=(0,0,0,0)).draw()
...
def game_screen(num_tries, num_score, letter_list):
	...
	hangman_box.blit(width-(hangman_box.width+20),letters_box.height+20) #hangman animation container
...
gameover=Text("Gameover. Your Score is: ", width//2, (7*height)//8, anchor_x='center', anchor_y='center', color=(255,144,0,255))
